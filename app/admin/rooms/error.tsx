...
export default function Error({
  ...
  useEffect(() => {
    console.error("Admin rooms page error:", error);
  }, [error]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Manage deal rooms between startups and investors.
          </p>
        </div>
      </div>
      ...
      <Card className="bg-[#0A0F1C] border-white/5 card-glow">
        <CardContent className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <p className="text-sm text-gray-400 text-center max-w-md mb-6">
            {error.message || "Failed to load deal rooms data. Please try again."}
          </p>
